...
async function fetchAvailableModels() {
  ...
  if (!apiKey) {
    toastr.error("请先配置API密钥", "获取模型失败");
    return;
  }

  try {
    $("#fetch_models_btn").prop("disabled", true).text("获取中...");
    ...
    console.log("筛选出的TTS模型:", ttsModels);

    extension_settings[extensionName].availableModels = ttsModels;
    saveSettingsDebounced();

    updateModelDropdown(ttsModels);

    if (ttsModels.length > 0) {
      toastr.success(`获取到 ${ttsModels.length} 个TTS模型`, "获取成功");
    } else {
      toastr.warning("未找到TTS相关模型，显示所有模型", "提示");
      // 如果没找到TTS模型，显示所有模型
      extension_settings[extensionName].availableModels = allModels;
      updateModelDropdown(allModels);
    }

  } catch (error) {
    console.error("获取模型列表失败:", error);
    toastr.error(`获取失败: ${error.message}`, "获取模型失败");
  } finally {
    $("#fetch_models_btn").prop("disabled", false).text("获取模型");
  }
}
...
function updateModelDropdown(models) {
  ...
  const currentValue = extension_settings[extensionName].ttsModel || modelSelect.val();
  ...
  if (models && models.length > 0) {
    modelSelect.append('<optgroup label="从API获取的模型">');
    models.forEach(model => {
      if (model.id !== "FunAudioLLM/CosyVoice2-0.5B") {
...
async function testSelectedModel() {
  const apiKey = extension_settings[extensionName].apiKey;
  const selectedModel = $("#tts_model").val();
  const testText = "这是模型测试语音，如果你能听到这段话，说明模型工作正常。";

  if (!apiKey) {
    toastr.error("请先配置API密钥", "测试失败");
    return;
  }

  if (!selectedModel) {
    toastr.error("请先选择一个模型", "测试失败");
    return;
  }

  try {
    $("#test_model_btn").prop("disabled", true).text("测试中...");
    ...
    if (voiceValue.startsWith("speech:")) {
      ...
      voiceParam = `${selectedModel}:${voiceValue}`;
    }
    ...
    console.log("测试模型请求:", requestBody);
    ...
    toastr.success(`模型 ${selectedModel} 测试成功！`, "测试通过");
    $("#model_test_status").text("✓ 模型可用").css("color", "green");

  } catch (error) {
    console.error("模型测试失败:", error);
    toastr.error(`测试失败: ${error.message}`, "测试失败");
    $("#model_test_status").text("✗ 模型不可用").css("color", "red");
  } finally {
    $("#test_model_btn").prop("disabled", false).text("测试模型");
  }
}
...
function updateVoiceOptions() {
  ...
  if (TTS_MODELS[model] && TTS_MODELS[model].voices) {
    ...
    // 对于未知模型，添加通用音色选项
    voiceSelect.append('<optgroup label="通用音色">');
    ...
    voiceSelect.append('<option value="charles">Charles (男声)</option>');
    voiceSelect.append('<option value="claire">Claire (女声)</option>');
    voiceSelect.append('<option value="david">David (男声)</option>');
    voiceSelect.append('<option value="diana">Diana (女声)</option>');
    voiceSelect.append('</optgroup>');
  }
...
async function generateTTS(text) {
  ...
  try {
    ...
    const modelValue = $("#tts_model").val() || "FunAudioLLM/CosyVoice2-0.5B";
    const voiceValue = $("#tts_voice").val() || "alex";
    ...
    if (voiceValue.startsWith("speech:")) {
      ...
      voiceParam = `${modelValue}:${voiceValue}`;
    }
